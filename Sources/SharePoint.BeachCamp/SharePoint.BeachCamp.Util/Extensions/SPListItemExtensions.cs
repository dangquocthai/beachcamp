...
using Hypertek.IOffice.Model.Security;
using System.Text;
using System.IO;
using System.Xml;
    ...
    public static class SPListItemExtensions
    {
        ...
        private static void CopyMetadata(SPListItem listItem, SPListItem destinationItem, string[] ignoreFields, SPFieldCollection destFields)
        {
            ...
            while (retry < 5 & !success)
            {
                        ...
                        if (sourceFields.ContainsField(destField.Title))
                        {
                            ...
                            if (!sourceField.ReadOnlyField && !sourceField.Hidden && !ignoreFields.Contains(destField.Title))
                            {
                                ...
                                catch
                                {
                                    ...
                                    CCIUtility.LogInfo("CopyMetaData " + destField.Title + " is not metadata of " + listItem.Title + ":" + listItem.Name, "Hypertek.IOffice.Common");
                                }
                            }
                        }
                    }
                ...
                {
                    CCIUtility.LogError(ex.Message + ex.StackTrace, "Hypertek.IOffice.Common");
                    Thread.Sleep(1000);
                }
            }
        }
        ...
        public static string GetFormulaValue(this SPListItem item, string formula)
        {
            ...
            if (matches != null)
            {
                foreach (Match match in matches)
                {
                    ...
                    string fielname = temp.Remove(temp.Length - 1, 1);

                    string fieldValue = string.Empty;

                    if (item.Fields.ContainFieldName(fielname))
                    {
                        fieldValue = item.GetStringValue(fielname);
                    }
